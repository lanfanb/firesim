...
object PlusargsBridge {
  ...
  private def annotatePlusargsBridge(clock: Clock, reset: Reset, conditionBools: Seq[Bool], params: PlusargsBridgeParams): PlusargsBridge = {
    ...
    val terminationBridgeTarget = Module(new PlusargsBridge(params))
...
class PlusargsBridgeModule(params: PlusargsBridgeParams)(implicit p: Parameters) extends BridgeModule[PlusargsBridgeHostIO]()(p) {
  lazy val module = new BridgeModuleImp(this) {
    ...
    override def genHeader(base: BigInt, sb: StringBuilder) {
      ...
      sb.append(genConstStatic(s"${headerWidgetName}_message_count", UInt32((params.defaultValue))))
